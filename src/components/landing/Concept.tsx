      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8 md:gap-12">
          {points.map((point) => (
            <div
              key={point.title}
              className="group p-6 sm:p-8 md:p-10 rounded-[1.5rem] sm:rounded-[2rem] md:rounded-[2.5rem] bg-[#F5F4DC]/30 border border-[#232323]/5 hover:border-[#AE343F]/20 hover:shadow-xl md:hover:-translate-y-2 transition-all duration-500"
            >
              <div className="mb-8 p-6 bg-white rounded-2xl inline-block shadow-sm group-hover:shadow-md transition-shadow">
                {point.icon}
              </div>
              <h3 className="text-xl sm:text-2xl md:text-3xl font-serif mb-6 text-[#232323]">
                {point.title}
              </h3>
